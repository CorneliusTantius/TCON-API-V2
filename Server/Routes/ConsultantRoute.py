...
async def ConsultantRegister(parameter: ConsultantCreateModel = Body(...)):
...
async def ConsultantLogin(parameter: UserLoginDTO = Body(...)):
...
async def ConsultantUpdate(Id: str, parameter: ConsultantUpdateModel = Body(...)):
...
async def ConsultantGetPaged(PageSize:int):
...
async def ConsultantGetById(Id:str):
